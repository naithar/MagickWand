...
apt-get update
apt-get install -y build-essential curl cmake zlib1g-dev libtool pkg-config
...
./configure --prefix=/usr           \
            ...
            --with-jpeg8            \
            --disable-static        \
            --docdir=/usr/share/doc/libjpeg-turbo-1.5.3 && make
make install
...
./configure --prefix=/usr    \
            ...
            --enable-lcms    \
            --disable-static \
            --docdir=/usr/share/doc/libraw-0.18.6 && make
make install
...
./configure CPPFLAGS='-I/usr/include:/usr/local/include:/usr:/usr/local' LDFLAGS='-L/usr/lib:/usr/local/lib:/usr:/usr/local'   --prefix=/usr --disable-static  --with-modules --without-magick-plus-plus --without-perl --without-magick-plus-plus --with-png=yes --with-quantum-depth=16 --with-gs-font-dir=/usr/share/ghostscript/fonts --disable-openmp --with-gslib=yes
make
make install
